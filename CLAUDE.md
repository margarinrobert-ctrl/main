...
**THE ALLOCATION BOOK CLEARS ZERO AND CLEARS NO NULL, AND THE TWO BEST-LOOKING ARMS HAD THE
LUCKIEST PATHS.** Both Monte Carlos on the book itself. Day-block bootstrap for the edge: **every
arm's 95% CI excludes zero**, P(mean<=0) 0.0000-0.0012 across all seven -- which is
`STUDY_V15_BOOK`'s split running the OTHER WAY, because the same arms read paired p 0.058 at best
against equal weight. Diversification is the established part; the choice of weights is not, and
both statements come from the same series. Permutation for the path, 4,000 reshuffles: mean-variance
sits at the **8.8th percentile** of reshuffles of its own trades and `top 5 x risk parity` at the
**7.8th**, against **48-53%** for the two un-selected equal-weight arms -- so exactly the arms with
the best return-over-drawdown are the ones whose drawdown was a draw rather than a property.
**MC p99 drawdown is 1.78x to 2.54x the realised**, and re-reading ret/DD there collapses the
selection advantage from 2.14x to **1.61x** (top5 x risk parity 14.90 -> 5.86, equal 6.97 -> 3.65).
The practical numbers for the best arm: **82.8% of days underwater and a longest underwater run of
339 days**. Read return-over-drawdown at the p99, not the realised, whenever the arm being praised
for it was also selected on it.

**A BOOK HAS NO TRADE-LEVEL PROFIT FACTOR, AND SELECTION CHANGES HOW IT EARNS RATHER THAN ONLY HOW
MUCH.** Eleven legs net against each other inside a day, so the only PF a portfolio has is over its
DAILY series and it is NOT comparable to the trade-level PF every other study here quotes -- print
both or the number is misread. Walk-forward day PF: all 11 equal **1.325 at a 47.9% day win rate
carried by a 1.44 payoff**; top 4 by training Sharpe **1.528 at 57.3% and a payoff of 1.14**. Same
family, opposite shape -- the losing legs were small frequent losses against the good legs' larger
wins, and removing them converts a PAYOFF book into a HIT-RATE book, while every weighting scheme
on all eleven sits at 47.5-48.1% and 1.43-1.47 because it only rescales the same shape.
**AND PF SEPARATES THE SELECTION MORE CLEANLY THAN SHARPE**: against 500 random allocators through
the same procedure (p5 1.187 / median 1.278 / p95 1.411) weighting tops out at **92.6** while top-4
reads **100.0**, and against 400 random SUBSETS of the same size re-drawn each fold top 3/4/5 read
98.0 / 99.8 / **100.0** against random medians of 1.22-1.27 -- because PF is not penalised by the
volatility concentration adds. **THE WRONG SHAPE IS INHERITED, NOT CREATED**: equal weight reads day
PF 1.235 research against 1.416 reserved, and the legs do the same (median leg trade-PF **1.089 ->
1.255**, nine of eleven improving, TRENDDAY/US100 1.119 -> 2.556). The three legs below PF 1.0 on
...
**AND ON THE BRIEF'S OWN DEFINITION -- 20 TO 150 POINTS, UP TO FOUR HOURS -- THE TRIGGER SEPARATES
FROM THE POPULATION FOR THE FIRST TIME AND STILL CLEARS NOTHING.** On US30's 31.17-pt in-window ATR
**20 points is 0.64N and 150 is 4.81N**, and four hours is exactly the 16-bar cap, so the geometry
called "not a scalp" above was inside the brief all along -- ASK WHAT THE UNITS ARE BEFORE DECIDING
A GEOMETRY IS OUT OF SCOPE. 168 declared cells (7 stops x 8 targets incl. none x 3 caps), entries
07:00-11:00 with the CAP binding rather than a bell: the population is **2.4% profitable** and
Donchian 20 long is **45.2%**, beating the population's marginal by +2.7 to +3.7 pts at every
setting. **THE MARGINALS POINT AT THE TOP OF THE STATED RANGE AND ARE MONOTONE**: stop 150 pts
+0.352 down to **20 pts -1.866**; target **none +1.897**, 150 +1.158, 100 +1.197, down to **20 pts
-3.837 at PF 0.741** -- a 20-point target is the single most destructive choice in the space; hold
4h > 2h > 1h, so the cap is at the GOOD end of its own axis. **AND THE POINTS PARAMETERISATION IS
ITSELF COSTING SOMETHING**: the same distances in ATR read **49.4% profitable / mean PF 0.9920**
against points' 45.2% / 0.9472, and the two DISAGREE about the stop -- points is monotone toward
wider (150 best) while ATR peaks at 1.28-1.60N and puts 4.81N WORST -- because a fixed 50-pt stop
was 4.23 ATR in 2016 and 1.10 in 2025 (`STUDY_DL50`), so a points grid confounds geometry with era.
Then the nulls: **0 of 7 triggers clear a matched random entry at the consensus geometry (0.4
expected, best p 0.260) and 0 of 8 geometries on the best trigger (best 0.280)**; the MEAN-REVERSION
fade outranks every breakout, the thirteenth route to that here; on research ALWAYS-IN BEATS THE
CELL (+1.548 vs +0.683); the day-block bootstrap reads P(mean<=0) **0.444 research / 0.631 holdout**
with a daily CI of [-7.86, +10.10] pts; and the block where it looks best (US30_ISO forward, PF
1.074, p 0.075) is the one opened LAST -- better out of sample than in, the wrong shape again.
Ship the SHAPE, claim no edge: wide stop, no target or the top of the range, hold the full four
hours, and size in ATR not points. See `docs/ib/STUDY_US30_SCALP_0711.md` sections 6-7.

**ANYTHING WORTH TRADING IN THAT WINDOW IS DETECTABLE AND ANYTHING UNDETECTABLE IS NOT WORTH
TRADING -- AND THE SEARCH'S OWN NOISE FLOOR EXCEEDS THE DETECTION THRESHOLD.** Diagnosing the null
rather than repeating it produced the governing number. `STUDY_V59`'s broken-null signature (excess
+3.04 pts failing at p 0.260) was checked and the null is SOUND -- sd(null mean)/sd(rule's own
standard error) **1.005-1.134**, null trade count stable at +-14 -- so what the p-value reports is
DISPERSION: **per-trade sd 157.1 points on a mean of +0.683**, i.e. **0.18 standard errors from
zero**. (Construction note for reuse: a control handed the rule's POST-lock count and then re-locked
settles ~34% below the rule's trade count, which is conservative, not invalid; over-draw by the
reciprocal of the rejection rate.) **THE POWER TABLE IS THE FINDING**: MDE at 80% power is **10.74
pts/trade** on the six-year block, while PF 1.2 requires **+10.61** and PF 1.5 **+23.62** -- so six
years is a LARGE sample for anything tradeable and small only for edges that are not; detecting the
observed +0.68 would take **1,528 years**. Then the decisive frame: over **1,176 declared cells**
the best t is **1.348** against the **2.802** detectability needs -- and against **E[max t | pure
noise] = 3.301** for a search that size. **The luckiest draw of a null search this wide looks more
convincing than a genuinely detectable edge**, so no configuration selected this way could be
trusted even if one had scored; 0 of 1,176 reach either bar, and best Sharpe 0.03377 sits an order
of magnitude below its own E[max|null] of 0.32031 at 1,401 counted looks. **AND THE RANKING
TRANSFERS WHILE THE LEVEL DOES NOT**: corr(research, holdout) over the 1,176 cells is **+0.6898
Pearson / +0.6965 Spearman**, far above this branch's usual -0.03 to +0.2, and selecting the
research top 1% lifts the holdout-profitable share **4.7% -> 45.5%, a 9.7-fold improvement, landing
at -1.339 pts a trade** -- a search working correctly with nothing to find, `STUDY_VP_DONCHIAN_US30`'s
split reproduced. **RANKING BY MEAN CHOSE THE NOISIEST CELL IN THE SPACE**: the mean-reversion fade
leads on points (+0.683) and is LAST on t (0.178) because its sd is 2.5x the Donchian's, while
corr(mean, t) across the grid is **+0.990** and corr(t, Sharpe) **+1.000** -- the RANKINGS are one
statistic wearing four names and only the TOP ROW differs, which is exactly why a top row is never
read. Best object, as a SHAPE with no edge claimed: Donchian 20 long, 30-pt stop, 150-pt target,
4h cap, 26.7% win at 1:5 on a 30-minute median hold -- research PF 1.107 control p 0.000 (chosen
from 168 geometries, so post-selection and unresolvable at 400 draws), holdout **0.919 p 0.425**,
forward 1.118 p 0.130. Event structure checked not assumed: **98.3% of sessions open ONE trade**, so
the rule is nearly "the first break after 07:00"; first-signal-only changes nothing (0 of 14);
concurrency max 1, mean 1.00. WHAT WOULD MOVE IT, IN ORDER: **pool the window across US100 and NQ**
-- three markets take the MDE 10.74 -> ~6.2 and bring PF 1.1 (+5.53) inside resolution for the first
time, and the feeds are on disk -- then 1-minute US30 bars, which fix the MEASUREMENT (the tie-break)
...
**THE RULE THE TEAM'S OWN EVIDENCE IMPLIED IS POSITIVE ON ALL SIX RESEARCH / HOLDOUT / FORWARD
CELLS, AND STILL INSIDE ITS MDE IN EVERY ONE.** Derived, not searched: the agents found ADX inverted
(ceilings 12/12) and the EMA ALIGNMENT carrying where the state does not, with ATR at chance, so the
implied rule is Donchian 20 long + an ADX CEILING and/or `ema13>34>89`, NO ATR condition, 07:00-11:00
with the 11:00 flatten. Ten declared research cells, the conventional `adx>=25` stack included as
THE ARM THAT MUST LOSE, then one read each of the holdout and the different-provider forward block.
**The prediction held at both geometries** -- 30/150 base +1.411 against +adx<=20 **+3.753**, +ema
**+3.106**, both **+4.972**, conventional +2.516; 50/150 base +0.956 against **+6.260 / +2.794 /
+6.613** and conventional +1.419. Across three blocks: **`+adx<=20` 6/6 cells positive at mean
+5.308 and `+ema align` 6/6 at +4.140, against the base's 4/6 at +1.790** -- and both turn a holdout
the base LOSES on (-1.086) into a profit. `adx<=20` at 50/150 clears its research control at
**p 0.010** with a day-block bootstrap of **0.037**, the only cell in the study to exclude zero,
while the conventional `adx>=25` stack clears **0 of 6** and is the only arm negative on the holdout.
**AND ALL THIRTY CELLS ARE INSIDE THEIR OWN MDE, 0 of 6 for every arm** -- the best (`+both` 50/150
holdout) delivers +8.51 against an MDE of **24.84**, and PF 1.2 still requires +10.61 against a best
delivered +6.61 research / +8.51 holdout; out of sample no cell clears a control (best p 0.070) and
the filters cut the count to 227 / 87 / 43, which is where the MDE goes. **THE FIRST REACHABLE
CONDITION NAMED ANYWHERE IN THIS FAMILY**: `+adx<=20` at 50/150 carries sd 73.2 on 400 trades for an
...
ARM RANKING INVERTS ON A DIFFERENT PROVIDER.** Four Monte Carlos, three blocks, a fixed-constant
walk-forward and three correlation matrices on `Donchian 20 long + adx<=20`, US30 07:00-11:00,
50/150, flat at 11:00. **`+adx<=20` is the only arm positive on all three blocks with a tight
spread** (+6.260 research / +4.596 holdout / +6.382 different-provider forward) while `+both`
(+6.613/+8.515/**-1.378**) and the conventional stack (+1.419/**-3.269**/+11.417) each swing 10-15
points between blocks. **MC: the rule survives everything that is not the sample** -- 250 execution
draws (slip U(0,2x), cost U(0.5x,2x) inside the walk) give P(<=0) **0.000** on both blocks, and 150
price-jitter draws with **ADX, both EMAs, ATR and the Donchian ALL RECOMPUTED** keep the sign
**1.000 / 0.993**. The bootstrap excludes zero on research (P(mean<=0) **0.037**) and does not on
the holdout (0.229). **AND THE PATH WAS UNLUCKY, NOT LUCKY** -- realised drawdown sits at the
**76th / 87th percentile** of reshuffles of its own trades, so MC p99 is **1.62x** the realised and
that is the sizing number. **THE WALK-FORWARD WITH CONSTANTS FIXED IS THE HONEST TEST FOR A DERIVED
RULE, AND THE RE-OPTIMISER LOSES AGAIN**: `+adx<=20` 6/8 folds at **+9.236** against re-chosen-each-
fold 4/8 at **+1.189** and a RANDOM arm at +3.817 -- fifteenth such comparison here, and the
re-chooser picked `conventional` twice and took -23.798 in 2020 for it. The fold table also shows
what no summary does: **2023 and 2024 are negative for EVERY arm** and **2025 alone supplies +36.253
for adx<=20 and +76.297 for +both**. **THREE MATRICES, THREE QUESTIONS**: between the ARMS,
`+adx<=20` correlates **0.052** with the conventional stack (opposite ADX readings) and **0.328**
with always-long, the lowest of any arm, so it is the least drift-like object in the set; between
the CONDITIONS on the signal bars, `adx<=20` vs `ema align` is **-0.1185**, confirming the team's
0.236 on a different construction, which is why the two stack at all; and across BLOCKS the arm
ranking is Spearman **A->B +0.900 but A->C -0.400 and B->C -0.700** -- **it transfers perfectly
between two blocks of the SAME feed and INVERTS on a different provider**, `STUDY_TREND_LONG`'s
lesson again, and precisely why `+both` looks best on A and B and dies on C. The battery removes
execution, data noise and path luck as explanations and identifies the arm that is most STABLE
...
**A SAME-SELECTIVITY NULL AND A FIXED-CONSTANT WALK-FORWARD RANKED THE SAME TWO CONDITIONS IN
OPPOSITE ORDERS, AND THE HONEST ANSWER IS THAT NEITHER DOMINATES.** Section 13's four Monte Carlos,
walk-forward and correlation matrices were run on `+adx<=20`; sections 14-15 demoted that arm, so
the battery was re-run on the promoted one rather than section 13's robustness being quoted for a
rule it never tested -- **an arm that has not been through the same tests is not a comparable arm**.
Result: on the null test `+ema align` is 18/18 and `+adx<=20` 15/18 (3/6 on the reserved feed), and
on **eight annual folds with the constants FIXED `+adx<=20` is the better arm** -- 6/8 positive,
+3,608 points, worst fold **-237**, against the EMA arms' 5/8, +2,128/+2,432, worst **-729**. So
section 14's "the more robust of the two" was true of one test and false of the other and is
corrected in place. The permutation says it from the other side: the EMA arm's realised research
drawdown sits at the **12th percentile** of reshuffles of its own trades -- a LUCKY path -- where
section 13 found the ADX arm's at the 76th and 87th, so part of the EMA arm's better
return-over-drawdown is a draw rather than a property. **Report which null a ranking came from; two
defensible tests on the same trades can order the same conditions oppositely.** One clean result
survives: `slow 34>89` correlates **+0.968** with the full alignment (the decomposition confirmed a
third way) while the ADX ceiling correlates only **+0.461** with either and both sit at +0.24/+0.32
against always-long -- two genuinely distinct non-drift conditions, whose CONJUNCTION section 13
already measured at **-1.378 on the reserved feed** with under 60 trades there, which is a
sample-size statement and not a mechanism one. Robust and not significant throughout: execution
...
**100 POINTS IS 2.35 ATR ON US30 AND 4.36 ON US100, SO A POINTS BARRIER IS A DIFFERENT STRATEGY ON
EVERY FEED.** Asked for a selectable 100-point TP and SL on the 09:00-range rule, the conversion
table was computed BEFORE the option shipped: 100 points is 2.35x the median in-window ATR on US30,
1.57x on US30_ISO, 3.24x on NQ, 4.36x on US100 -- and on US30 alone it was 4.23 ATR in 2016 against
1.10 in 2025 (`STUDY_DL50`), so a points grid confounds geometry with market AND with era. At
100/100: US30 research +0.0031 %/trade PF 1.022 at a **51.7% win rate against a driftless
break-even of 51.1%** -- the barriers are hit by noise, fourth family -- then holdout **-0.0164
PF 0.872 with a random entry of the same geometry beating it at p 1.000**. Across 14 cells on four
feeds **1 clears p<=0.05 where 0.7 are expected and 0 of 14 exceed their own MDE**. Over a 75-cell
declared points grid on US30 research (54.7% profitable, best |t| 3.635 against `E[max t | noise]`
2.428) the TARGET marginal is MONOTONE -- none +0.87 bp, 200 pt +0.80, 150 +0.43, 100 -0.44,
**50 -1.07, the single worst choice in the space** -- so NO TAKE PROFIT wins for the 26th time and
the points parameterisation says it independently; the STOP marginal runs the other way (50 pt
-0.26 to 200 pt +0.42), the monotone-toward-wider result for the ninth family. The tie-break
decides nothing at these widths (ambiguous share 1.33% at 50 points, 0.01% at 200). Parity 10/10
...
**THE 09:00 BAR AND THE 09:00-09:30 HALF HOUR ARE THE SAME RULE, AND THE DELTA IS 42x SMALLER THAN
THE SAMPLE CAN RESOLVE.** Asked to mark "the 9am high and low", the two readings were paired on
identical feed / block / side / geometry with the entry armed at 09:30 in both: **the 09:00 bar wins
14 of 28 cells, exactly chance, mean delta +0.0014 %/trade against a mean MDE of 0.0593**. What DOES
change is the LEVEL -- the one-bar range is 1.03-1.13 ATR wide against the half hour's 1.50-1.87, so
the breakout sits about a third nearer and fires slightly more often. On the one-bar default US30
reads +0.0145 / +0.0098 / +0.0087 %/trade long across research / holdout / a different-provider
forward block -- positive on all three and on both sides, with **3 of 56 declared cells clearing
their control where 2.8 are expected and 0 OF 56 outside their own MDE**. A window ambiguity in the
ASK was worth nothing; the same ambiguity in an INPUT was worth a different strategy -- see below.
...
**AN AUTO BREAKEVEN SUBTRACTS ON EVERY RUNG PAST 25 POINTS, AND ITS OFFSET MOVES A WIN RATE
THIRTY-ONE POINTS WITHOUT MOVING THE MONEY.** Asked for as a 50-point option on the 09:00-range
rule, and measured rather than wired up because `TEAM_EXIT_PF` had already found
breakeven-after-1R the one exit policy that subtracts. Declared ladder: off / 25 / 50 / 75 / 100 /
150 points x long/both x two geometries x US30 research, holdout and a different-provider forward
block -- **72 NOMINAL cells and 60 EFFECTIVE**, because at a 100-point target a breakeven armed at
100 or 150 can never fire (the target resolves first on the same bar) and those twelve rungs
reproduce their OFF twin **to the cent and to the exit bar, 0 of 12 differing** -- checked, not
asserted. `E[max t | noise]` over 60 looks 2.345. Paired against each cell's own OFF twin the
marginal average is **+0.0036 at 25 points, -0.0008 at 50, -0.0034 at 75, -0.0034 at 100, -0.0038
at 150 %/trade**, and a breakeven beats its own twin in **19 of 60 cells = 32% where chance is
50%**. THE MECHANISM IS IN THE EXIT MIX AND IS THE SAME EVERYWHERE: the stop share rises and the
flatten share falls by almost exactly as much (at 50 points +0.145 / -0.095), so the ratchet arms
on a favourable excursion and is taken out on the pullback before the clock would have closed the
trade in profit. **AND THE SECURED-POINTS OFFSET IS THE CLEANEST WIN-RATE ILLUSION MEASURED HERE.**
A TRUE BREAKEVEN STOP IS A LOSING TRADE BY CONSTRUCTION -- an exit at the fill books MINUS the
round turn -- so the threshold that turns it into a win is THE ROUND TURN, NOT ZERO (US30 2.29
points, so a 5-point secure books +2.71). Across three blocks at breakeven 50, securing
5 / 10 / 25 points moves the win rate **+0.410 / +0.410 / +0.411** while moving the TRADE COUNT by
**0 to 5 trades out of 268-1,799** -- it is overwhelmingly RELABELLING exits it did not move
(research long 0.2207 -> 0.5275 win at +0.011307 -> +0.009270 %/trade; forward block 0.1493 ->
0.6493 at +0.0098 -> +0.0110). READ THE TRADE COUNT BEFORE THE WIN RATE. The money ladder rises
with the distance (-0.0008 / +0.0006 / +0.0059, 2/6, 4/6, 5/6 cells) and that is a WARNING not a
recommendation: a secured level far enough from the fill has stopped being a breakeven and become
a small take profit, which has lost 26 times here. And compute the round turn LIVE rather than
assuming it -- `(gross - net) / trades / point value + 2 x tick`, because slippage is charged in
TICKS on both sides and never appears in the commission figure. Every paired delta is INSIDE its own MDE (0 of 60 outside) and **0 of 72 cells clear a
matched random entry** where 3.6 are expected, so it is a consistent direction and not a resolved
effect. Ships as three inputs, DEFAULT OFF with 50 pre-filled; in Pine the ratchet is re-issued as
an ABSOLUTE stop from `strategy.position_avg_price` once the position exists, which is one bar
after the order is written and exactly when the research walker arms. Parity 12 of 12 cells at
trade count 1.000, same exit bar 0.979-0.997, gap -0.39 to +0.07 pts/trade.

**`runlog.sh <logfile> <command>` -- REVERSING THOSE TWO ARGUMENTS TRUNCATES YOUR SCRIPT.** It does
`: > "$LOG"` on `$1`, so `runlog.sh research/x/run_n7.py /tmp/n7.log` empties `run_n7.py` and then
writes stdbuf's error message into it; the next run fails with a SyntaxError pointing at a file
that reads nothing like the one you wrote. Log file FIRST, always. Tenth self-inflicted
name/argument collision on this branch.

**THE 08:00 HOURLY CANDLE IS THE SECOND CONDITION ON THIS TRIGGER TO PASS THE BASE-RATE CHECK, AND
IT CLEARS 0 OF 36 NULLS.** Asked for as a direction gate -- a bearish 08:00-09:00 New York hour
takes only the bearish break. It is CAUSAL BY CONSTRUCTION (the hour completes at 09:00, before the
09:00 range starts and ninety minutes before the 09:30 arm) and audited anyway: **0 mismatches of
40 probes** on both feeds. And it is NOT the trigger restated -- it admits **0.376 to 0.543** of the
breaks it is shown across three readings x two sides x two feeds, dead centre, against the
confirmation family that has failed this check eight times (RSI 94.7%, Aroon 100.0%, MACD 99.8%,
MFI 91.7%, +DI 97.8%, close>EMA50 93.7%, EMA13>48-on-Donchian 82.6%). **THEN IT FAILS EVERYTHING**:
36 declared cells (3 readings x ALIGNED/COUNTER x two geometries x US30 research, holdout and a
different-provider forward block) against a same-selectivity random GATE re-simulated end to end --
**0 of 36 clear p<=0.05 where 1.8 are expected, 0 of 36 exceed their own MDE, and the gate beats
the ungated rule in 12 of 36 cells where chance is 50%**. ALIGNED, the literal ask, is NEGATIVE on
all three blocks (-0.0041 / -0.0045 / -0.0042 %/trade); COUNTER is positive on research only
(+0.0031) and negative on both blocks it did not choose -- and on the 1.5N geometry the research
block flatters it +0.0171..+0.0242 against a +0.0067 baseline before reading -0.0099..-0.0168 on
the holdout, which is why the mirror was DECLARED in advance rather than discovered afterwards.
**PASSING THE BASE-RATE CHECK IS NECESSARY AND NOT SUFFICIENT, MEASURED TWICE IN ONE STUDY.** Ships
default OFF with the hour accumulated from the chart's own bars and frozen at 09:00, never pulled
with `request.security(..., "60")` -- a 60-minute security bar is the EXCHANGE's hour and need not
begin at 08:00 New York, the same reason an RTH session high has to be accumulated here.
Parity 15 of 15 configs at trade count 1.000. See `docs/ib/STUDY_NINE_AM_RANGE.md` section 14.

**A 200-PERIOD AVERAGE IS SO SLOW THAT A 15-MINUTE BREAKOUT CARRIES NO INFORMATION ABOUT IT — LIFT
EXACTLY 1.00, WHICH IS THE OPPOSITE FAILURE FROM THE TRIGGER-RESTATED ONE.** Asked to add the MA200
to the 09:00-range script and require the provided EMAs to have crossed it on the break's side.
Base rates on the trigger's own bars FIRST: `13 or 48 above the 200` admits **0.6155** of long
breaks against 0.6190 of all bars, `13 and 48 above` 0.5411 against 0.5389 — **lift 0.990 to 1.007
across every state cell on both feeds and both sides**, against `close > MA200`'s 1.089-1.152 on
the same bars. The information is in where PRICE is, not in where the slow averages are, so the
gate is a coin flip applied to the signal set. Nine confirmation families have now died on this one
check and this is the FIRST to die from the other end: RSI 94.7%, Aroon 100.0%, MACD 99.8-100.0%,
MFI 91.7%, +DI 97.8%, close>EMA50 93.7%, EMA13>48-on-Donchian 82.6% and VWAP-vs-stochastic rho
+0.831 were all the trigger restated; this one is independent to the point of carrying nothing.
**AND `ANY` AND `ALL` ARE THE SAME CONDITION UNLESS A SPLIT READING RESOLVES** — read as a single
+1/-1/0 direction label the two modes are IDENTICAL TO FOUR DECIMALS (0.5389 / 0.3810 / 0.0801 for
both), because "at least one above with none below" IS "all above"; the axis binds only when the
gate returns TWO MASKS and a split day (13 above, 48 below) confirms both sides. Collapsing it
would have counted twelve tests never run — `run_n7`'s inert-rung accounting from another
direction. Then the grid: 48 declared cells at `E[max t | noise]` **2.261** against the 2.802
detection needs, scored as a VETO against a same-selectivity random gate re-simulated end to end.
**3 of 36 scorable cells clear p<=0.05 where 1.8 are expected, 0 OF 36 EXCEED THEIR OWN MDE, and
the gate beats the ungated rule in 15 of 36 where chance is 50%.** ALIGNED -- the literal ask -- is
**negative in 0 of 6 cells for BOTH state readings** (-0.0189 and -0.0199 %/trade against no
filter); the COUNTER mirror carries all three passing cells (+0.0085 / +0.0113) and reads
**-0.0077 on the reserved different-provider forward block**, so the polarity that scores is the
one that dies where nothing chose it. 12 of 48 cells are UNSCORABLE because `all CROSS` admits
0.0-1.1% of breaks. Ships as four options on the existing MA input, DEFAULT OFF, with the
base-rate table in the tooltip. Parity 20 of 20 configs at trade count 1.000, same exit bar
0.974-1.000. See `docs/ib/STUDY_NINE_AM_RANGE.md` section 15.

**A TREND LINE IS A LEVEL THAT MOVES, AND USING IT AS THE BREAKOUT LEVEL LOSES TO A RANDOM ENTRY AT
p 1.000.** Asked to add trend lines through at least two pivots and break them. Three declared
readings: GATE (keep the 09:00-range break, additionally require price to have cleared the line),
LEVEL (the line IS the breakout level), EITHER (whichever level is nearer). Construction is the
whole risk and it is audited rather than argued -- a pivot at bar j needs j-prd..j+prd so the line
may only be anchored at j+prd, `STUDY_DIVERGENCE_CONFIRM`'s leak, and the truncation audit reads
**0 mismatches of 48 probes on both feeds**. **THE BASE-RATE CHECK PASSES, THE THIRD TIME IN THIS
STUDY**: the gate admits 37.2-40.0% of the trigger's own bars at two points and 6.8-10.7% at three,
lift **1.30 to 2.71** -- a genuinely separate reading, against the nine confirmation families that
died by passing 82-100% and the MA200 cross that died at lift exactly 1.00. Then 48 declared cells
(4 modes x 2/3 points x two geometries x three blocks) at `E[max t | noise]` 2.261: **2 clear
p<=0.05 where 2.4 are expected, 1 OF 48 EXCEEDS ITS OWN MDE, and a reading beats the ungated rule
in 27 of 48 where chance is 50%**. **LEVEL IS THE WORST OF THE FOUR AND IT IS THE LITERAL ASK** --
on US30 research it takes MORE trades than the range (1,863 against 1,734) and loses to a matched
random entry at **p 1.000**, so an extended line is a worse place to enter than an arbitrary bar in
the same session; median distance from the close to the line is **2.1-2.5 ATR**, which is what
"extended" costs. GATE has the best marginal (+0.0034 at two points, +0.0120 at three) and INVERTS
by block (+0.0186 research, **-0.0171 holdout**, +0.0216 forward), and the three-point form reads
the largest number in the table on ~120 trades a block against an MDE of 0.12-0.14 -- the least
resolvable cell in the study. **AND THE TWO READINGS NEED DIFFERENT NULLS**: a GATE is a veto and
takes a same-selectivity random gate, while LEVEL and EITHER change which bars the rule fires on at
all, so a selectivity control is the wrong null for them and a matched random ENTRY is the right
one -- scoring all four against one null would have flattered the two that changed the trigger.
Ships default OFF; parity 25 of 25 configs at trade count 1.000, same exit bar 0.974-1.000.
See `docs/ib/STUDY_NINE_AM_RANGE.md` section 16.

**A 1:1 BARRIER PAIR MAKES THE SECURED-POINTS ILLUSION FIFTY-THREE POINTS WIDE, AND A BREAKEVEN
THAT "HELPS" IS A DIFFERENT STRATEGY WITH MORE TRADES.** A submitted MA 13/48 cross with fixed
100-point barriers on US30 (one position, no reversal, MA200 plotted and never traded), given the
three additions asked for: a session window, a session stop and a breakeven. **The rule is a null
and its win rate is its own break-even** -- symmetric barriers at a 2.29-point round turn need
51.15% and it delivers **51.28% research / 45.63% holdout / 51.86%** on a different-provider
forward feed, so the barriers are hit by noise (fifth family after THE_STRAT, IB25, VWAP_STOCH_ATR
and V69_ORB). **SESSION, 42 declared cells at `E[max t | noise]` 2.209**: 13:00-16:00 is the ONLY
positive window (+0.83 pts, PF 1.014) against all hours -3.11, 09:30-16:00 -2.58, 09:30-11:00
-3.01, 07:00-11:00 -6.11, 08:00-12:00 -9.04 -- the one positive window is the only non-morning one,
the EIGHTH session preference here to disagree with the previous one -- and **the session stop
helps in 9 of 18 paired cells, exactly chance, at -0.67 pts/trade**. **BREAKEVEN, 21 EFFECTIVE
cells** (rungs at or beyond the target are INERT because the target resolves first on the same bar,
excluded rather than counted): it beats its OFF twin in 13 of 18 paired cells, monotone at +4.35 /
+1.43 / -0.22 pts at 25 / 50 / 75 -- **and the TRADE COUNT RISES BY 251 / 118 / 46**, because
closing sooner frees the one-position lock and admits later crosses the base never saw. That is
STUDY_V56's mechanism, and it means the ladder is not a filter on the same trades. **AND THE
SECURED OFFSET IS THE LARGEST WIN-RATE ILLUSION MEASURED ON THIS BRANCH**: at breakeven 25 the SAME
trades read **26.07% securing 0 and 79.35% securing 5 -- fifty-three points** -- while the money
moves +3.099 to +3.472 pts a trade, against the 09:00-range rule's 31 points, because a 1:1 geometry
puts far more mass on the moved stop. Nulls: against a random entry with identical window, side mix,
barriers, breakeven and cost, **2 of 9 declared cells clear p<=0.05 where 0.5 are expected and BOTH
are research cells** (13:00-16:00 p 0.024, breakeven 25/5 p 0.004) reading 0.308 / 0.500 on the
holdout and 0.288 / 0.268 on the forward feed; **1 of 9 exceeds its own MDE**. All three additions
ship DEFAULT OFF with their own numbers in the tooltips. Parity 14 of 14 configs at trade count
1.000, gap +0.000 to +0.284 pts/trade. See `docs/ib/STUDY_MA13_SESSION_BE.md`.

**AN OPPOSITE-EMA-CROSS EXIT IS A COIN FLIP CLOSING EARLY, AND THE PLACEBO IS WHAT SHOWS IT.**
Asked for on the 09:00-range breakout, measured over 72 declared cells / **48 scorable** (the OFF
arm never reads the MA pair, so its two pair rows are ONE cell -- counting them twice corrects the
multiplicity for 24 tests never run). It beats its own OFF twin in **15 of 48 = 31% where chance is
50%**, **0 of 48 paired deltas exceed their own MDE**, both readings are negative on the marginal
(fresh cross -0.0010 %/trade, opposite state -0.0044) and both MA pairs agree in sign, so it is a
direction rather than a spike. **THE BINDING RATE IS THE FIRST THING TO PRINT**: the FRESH CROSS
closes only **4.5-7.5%** of trades -- a 13/48 flip inside a 2.5-hour hold is rare, so it is nearly
an inert switch -- while the OPPOSITE STATE closes **35%** and cuts the median hold 10.9 bars to
6.5, taking US30 research long from +0.0145 %/trade at a **32.6%** win rate to +0.0076 at **38.9%**
and the holdout from +0.0098 PF 1.053 to **-0.0101 PF 0.907**: a payoff book converted into a
hit-rate book, paying for the conversion. **AND THE PLACEBO IS THE FINDING** -- re-run against an
exit array with the SAME NUMBER of early exits at RANDOM bars (100 seeds a cell), the real delta
sits at mean percentile **0.38 / 0.57** of its own null, **the placebo does BETTER in 12 of 24
cells**, and 3 of 24 clear the 95th percentile against 1.2 expected. Closing early at that RATE
costs what closing early AT THE CROSS costs; the moving average contributes nothing. A before/after
cannot separate those two and **every early-exit policy needs this test** -- it is the
random-delay placebo of the execution-overlay literature applied to an EXIT. One row is the trap:
on research long the state reading beats its placebo at percentile **1.00** and its delta is still
**-0.0069** -- it beats a null that loses more, which is `STUDY_IB_US30_OPTUNA`'s sentence from the
other side. Fourth exit policy measured as a wash or worse here after the ATR trail, the channel
exit and breakeven-after-1R. Ships DEFAULT OFF. Parity 30 of 30 configs at trade count 1.000, same
exit bar 0.986-1.000, and the gap is NEGATIVE on every cross-exit config (-1.06 to -2.19 pts a
trade), the conservative direction. See `docs/ib/STUDY_NINE_AM_RANGE.md` section 17.

**AN ATR TARGET IS THE R TARGET RENAMED UNTIL THE STOP STOPS BEING AN ATR MULTIPLE, AND THE ATR
PERIOD IS A HUMP CENTRED ON THE INCUMBENT.** Two additions to the 09:00-range script, both measured
before shipping, and three gates deleted rather than left switched off (the 08:00 hour, the S/R
channels and the trend lines -- 0 of 36, opposite signs on two blocks of one market, and 1 of 48
outside its own MDE; the measurements stay in the study, only the switches go). **THE IDENTITY IS
ASSERTED, NOT REASONED**: `k x ATR` and `r x stop` are the same level when `k = r x stopAtr`, and on
2,295-2,580 trades at 1.5N and 2.5N the two give IDENTICAL trade counts, IDENTICAL exit bars and
**max |dpts| 0.000e+00** -- they separate only under a POINTS or range stop, where they agree on
just **66-73%** of exit bars. So an ATR target is a new axis outside the default geometry and a
rename inside it; `STUDY_V40`'s duplicate-catch reached from the other direction, and worth building
into any two-parameterisation comparison. **THE ATR PERIOD**: 60 cells paired against their own
ATR(14) twin at `E[max t | noise]` 2.345 -- a rung beats 14 in **21 of 60 = 35% where chance is
50%**, 0 of 60 exceed their own MDE, and the marginal runs -0.0027 / -0.0019 / +0.0004 / -0.0002 /
-0.0021 at 7/10/21/30/50, a smooth hump with the only non-negative rungs either side of the
incumbent. **AND THE TRADE COUNT MOVES WITHOUT THE EVENTS MOVING** -- it rises monotonically with the
period (-15 at 7, +13 at 50) at a zero break buffer, where the signal set is identical by
construction, because a wider stop holds longer and the one-position lock then refuses later breaks:
`STUDY_V56`'s mechanism reached from the STOP rather than the exit, and a reminder that a count
change is not by itself evidence a filter fired. **THE ATR TARGET LOSES MONOTONICALLY** -- paired
against no target the marginal is **-0.0167 %/trade at 1.5 ATR (1R)**, -0.0115 at 2.25, -0.0070 at
3.0, -0.0042 at 4.5 and +0.0020 at 6.0, beating no target in **8 of 30 cells**, 0 of 30 outside
their MDE -- and the one non-negative rung has a **9.4% target-hit rate**, so setting 6 ATR and
setting NONE are nearly the same strategy (`STUDY_INTRADAY_HEAT` on a different base). 27th time no
take profit has won here. Both ship at their researched defaults (period 14, target None). Parity
28 of 28 configs at trade count 1.000. See `docs/ib/STUDY_NINE_AM_RANGE.md` section 18.

**A 30-SECOND US30 FEED ARRIVED AND IT ANSWERS THE TIE-BREAK, NOT THE SAMPLE SIZE.** `US30_30s`:
390,552 bars 2025-08-18 to 2026-09-16, the **tenth export format and the first PARQUET**
(ClickHouse 26.6.1), and the finest US30 series here by a factor of thirty -- the file
`STUDY_US30_SCALP_0711` named twice as the first thing that would move its verdict. Four things
checked before it was used. **IT IS STAMPED IN UTC AND THE COLUMN SAYS SO**, which is `NQ_1m`'s
trap on a market where every other file is broker time at NY+7 -- re-derived anyway rather than
taken from the dtype: after `tz_convert('America/New_York')` mean bar range peaks at minute-of-day
**570 = 09:30 New York** (74.69 points against 26.65 one bar earlier). **VOLUME IS REAL AND ONLY
FROM 2026-04-27** -- identically zero on the first 134,655 bars, then present on every bar at
`corr(volume, high-low)` **+0.7001**, inside the genuine feeds' +0.71..+0.77 band and nothing like
`XAUUSD15_MT`'s +0.0048 fake column, so 4.5 months of usable volume in a 13-month file.
**BARS WITH NO ACTIVITY ARE OMITTED** -- 390,552 of 1,135,775 possible slots = **34.4%** coverage,
92% inside 10:00-16:00 and 31% overnight with the 17:00 hour absent entirely, so a BAR INDEX IS
NOT A CLOCK and the export dialog's own 812,571-row estimate is wrong by 2.1x. And **IT IS A THIRD
US30 PROVIDER, WHICH CUTS BOTH WAYS**: against `US30_ISO_15m` on 11,882 shared 15m stamps it reads
corr 0.999926 and level ratio 1.000463 but mean |diff| **26.8 POINTS** at an exact-match share of
0.03-0.07% (chance), so it is not a re-upload -- and it overlaps US30_ISO from 2025-08-18 to
2026-08-26 with only **2026-08-27 onward** post-dating every other US30 file, so over the shared
span it is a FEED-PARITY check and not a second test (US30/US30_ISO already at daily leg
correlation +0.922). **THE TIE-BREAK, ON THE SAME TRADES AT THREE RESOLUTIONS**: 11.65% of 0.5N
trades are ambiguous on 15m (against the study's 14.03% on its own provider -- two feeds, not a
correction) and **92.63% of the 95 ambiguous trades resolve at 30 seconds**, splitting **stop
first 61.4% / target first 38.6%** -- so **the branch's stop-always convention is right about six
times in ten, not ten**: conservative as intended, but over-charging on 38.6% of that population
rather than none of it, which is how one study reported PF 0.604 against 1.056 for the same trades
with neither number being the market's answer. Above 1.0N the question is nearly moot (0.37%, then
0.00%). **AND 1-MINUTE BARS WOULD HAVE DONE ALMOST ALL OF IT** (95.24% against 96.83% at 0.5N,
+1.6 points for a 2x finer series) -- what unlocked the measurement was going below 15 minutes at
all. It does NOT move the power table: a four-hour cap holds the rate near one trade a session
whatever the bar size, so the MDE is still 10.74 pts/trade and the 1,073-trade requirement stands.
...
| `research/pm/` | **the portfolio question, asked as a desk would ask it**: `pm_book.py` (PER-LEG AVAILABILITY -- a leg enters the book only after its OWN research block ends, which admits all 23 legs and makes every daily return out of sample for the leg that produced it, plus causal inverse-vol weights, the day-block bootstrap and the path permutation), `run_p1.py` (the construction defect priced against the published 11-leg book, and the leverage-vs-drawdown table that a return target actually picks from), `run_p2.py` (Sharpe against legs live, the full-book era, leg drop-one, and the `s*sqrt(N)/sqrt(1+(N-1)rho)` ceiling with solo Sharpe ZERO-FILLED as it must be), `plot_pm.py` |
...
| `pine/macross/MA_CROSS_SELECT_strategy.pine` + `research/macross/mx_sim.py` | a plain fast x slow crossover with the TYPE selectable on each side (SMA/EMA/WMA/RMA/HMA/VWMA/LinReg/DEMA/TEMA), plus two default-OFF options -- a 09:00 NY range confirmation (cross closes beyond it, or cross and break within N minutes either order) and a points breakeven ratchet copied from the 09:00 script; the sim is the script's order model run on bars as a mechanics check, with LinReg asserted equal to 3*WMA-2*SMA at 5e-11 and the range's base rate on cross bars printed (lift 1.08-1.42 on US30_30s, only 92 of 293 sessions carrying the range) |
...
**THE PROFITABILITY GOAL WAS HELD DOWN BY A BOOKKEEPING CHOICE, AND THE BOOK THAT CLEARS ZERO WAS
BUILT ON 11 OF 23 LEGS.** Asked as a portfolio manager rather than a researcher. THREE LAYERS, each
already measured here: (1) ARITHMETIC -- every leg earns +0.04 to +0.08 R while the MDE at any
realistic trade count is LARGER than that, so **no single-strategy search on this data can confirm a
single strategy**, and more searching makes it worse by raising the noise floor the winner must
clear; (2) THE QUESTION -- a RESEARCH question at the LEG level answers no every time while the
PORTFOLIO question at the BOOK level answers **P(mean<=0) 0.0000-0.0012**, and both are correct;
(3) CONSTRUCTION -- `STUDY_ALLOCATION`'s GLOBAL research cut (`min` over legs = 2021-12-17) is set by
ONE leg, every NQ leg begins a year after it, so the one object here that clears zero **excluded
FTM_ORB/NQ and V56_CVD/NQ, the two highest-scoring strategies in `STUDY_TOP5`**, for a calendar
reason and not an evidential one. **THE FIX IS THE HONEST CONSTRUCTION ANYWAY**: give every leg its
OWN availability date -- it may enter only after its own research block ends, which is what a desk
does -- admitting all 23 legs and REMOVING the caveat STUDY_ALLOCATION had to attach to its own
headline, since under per-leg availability every daily return is out of sample for the leg that
produced it. Worth **+0.16 Sharpe (1.451 -> 1.611)** and +13% of return on the same calendar with
nothing fitted: **+3.70%/yr at 2.30% vol, ret/DD 7.56, bootstrap P(mean<=0) 0.0010**.
**AND THE RETURN IS A LEVERAGE DECISION, NOT A STRATEGY DECISION** -- 15%/yr needs **4.05x** and
implies an **MC p99 drawdown of 15.85%** (the p99 is 1.72x realised; size against that column).
No configuration search anywhere in this repository changes that table; only rho and N do.
**THE CEILING IS SET BY rho AND IT IS ~1.9**: mean pairwise leg correlation **+0.0387**, mean solo
leg Sharpe zero-filled **+0.380**, so `s*sqrt(N)/sqrt(1+(N-1)*rho)` gives 1.34 at N=23 (measured
1.61) and **1.82 at N=200** -- adding more legs OF THIS KIND cannot pass ~1.9 however many are
added, and **the lever is rho, not N**. A genuinely independent asset class is worth more than any
number of further equity-index strategies; the registry names three absent from disk (XAUUSD 5m,
EURUSD 30m, BTCUSDT 15m). **AND I MADE THE ZERO-FILL ERROR IN THE VERY CALCULATION THAT EXISTS TO
PRICE IT**: measuring solo leg Sharpe on ACTIVE DAYS ONLY gave +1.132 and predicted a book Sharpe of
**3.99** against a measured 1.61; zero-filled it is +0.380 (a factor of **2.98**) and the prediction
lands. Caveats that stay attached: this is a DESCRIPTIVE re-read of spent blocks, not a
pre-registered test; **Sharpe 1.611 +- 0.527, 95% CI [0.58, 2.64]**, and a Sharpe standard error
falls only with TIME; the full-book era (>=20 legs live) is **414 days at Sharpe +1.199 +- 0.781**;
**85.1% of days are underwater**; and drop-one says 7 of 23 legs subtract, the worst three all US30,
reproducing STUDY_ALLOCATION's selection. See `docs/ib/STUDY_PM_BOOK.md`.


**AN "OR" LOOSENS A GATE, SO IT CAN ONLY BE READ AGAINST BOTH ARMS IT SITS BETWEEN -- AND THE
LOOSEST ARM WON.** Asked to let a break AT the EMA200 enter without the MA cross ("the same as the
ema 200 as support/bullish or resistance/bearish"), which is an OR and therefore sits between "the
MA gate alone" and "no gate at all"; read against the gated arm only it looks like an improvement
whenever the gate was worth nothing, which on this trigger is what two earlier sections had already
found. Declared: 3 tolerances x 3 readings x 2 geometries x 3 blocks = 54 strict cells plus 36 OR
cells, `E[max t | noise]` **2.493** against the 2.802 detection needs. **IT PASSES THE BASE-RATE
CHECK, THE SECOND CONDITION IN THIS STUDY TO DO SO**: confluence admits **5.2-28.0%** of the
trigger's own bars at lift **1.25-1.63**, against nine confirmation families that died there (RSI
94.7%, Aroon 100.0%, MACD 99.8-100.0%, MFI 91.7%, +DI 97.8%, close>EMA50 93.7%, EMA13>48 82.6%,
stochastic-vs-VWAP rho +0.831, and the MA200 CROSS at lift exactly 1.00) -- **a bypass has TWO ways
to be worthless and they are opposite ends of one table: fire almost never and it is an inert
switch, fire almost always and it IS "no gate"**. Then: as a REQUIREMENT **2 of 50 scorable cells
clear a same-selectivity random gate at p<=0.05 where 2.5 are expected, 0 OF 50 EXCEED THEIR OWN
MDE**, and it beats the ungated rule in 20 of 50 where chance is 50%. **THE READING THE ASK NAMES
IS THE BEST OF THE THREE AND IS ONE CELL**: the average BEHIND the break (support on a long,
resistance on a short) at 0.25 ATR is the only positive marginal (+0.0268 %/trade) and its single
clearing cell is US30 research PF **2.029** p 0.043 on 82 trades, reading **PF 0.523 p 0.803** on
the holdout with a research effect of +0.1438 INSIDE its own MDE of 0.2107; both other readings'
marginals get monotonically WORSE as the tolerance widens, the shape of a condition whose only
content is selectivity. As the OR itself it beats the MA gate in **13 of 36** cells and **no gate at
all in 4 of 36**, 0 of 36 paired deltas exceed their MDE, and the pooled arm means run no gate
**-0.0025**, MA gate -0.0123, confluence only -0.0123, **MA-or-confluence -0.0188** -- the least bad
OR row is the LOOSEST one (1.00 ATR, +360 signals), i.e. the nearer the bypass takes the rule to no
gate the better it does. **The bypass is a slower way of switching the gate off and switching it off
is free.** Mechanic: in a parity harness an OR's two masks must be combined BEFORE either drops a
signal -- gating in sequence is an AND, a different strategy. Ships as two inputs, DEFAULT OFF.
Tenth self-inflicted name collision here (`k = mk_ma | mk_cf` shadowed the `enumerate` index and
