...
## Final selection (`final_selection.py`)

The last gaps closed before shipping a "best version":

**1. Sizing stability across W.** The martingale study above sized only the
W=1 variant. Re-running the sizing Monte Carlo (10,000 trade-bootstrap paths
per pair) for *every* long-only W 1..30 changes one conclusion:

| scheme | plateau (W 4–13) median CAGR | plateau median MC maxDD | **worst-case P(ruin) over all W** |
|---|---|---|---|
| fixed 1x | 17.0% | −18.3% | 1.1% |
| split-entry 1/3 @1x | 12.4% | −14.7% | **0.2%** |
| split-entry 1/3 @2x | 23.4% | −29.3% | **21.6%** |

The @2x split entry — previously blessed with "P(ruin) 1.6%" — held that
number *only at W=1*. Across the parameter space its ruin probability runs
3.8% (plateau median) to 21.6% (worst W). **Not robust; the shipped default
is 1x.** (`SemivarianceContrarian.pine`'s default was corrected from 200%
to 100% accordingly.)

**2. Candidate bake-off** (identical data, 10,000-path daily block bootstrap):

| candidate | Sharpe | CAGR | maxDD | t | ex-COVID | OOS ≥2022 | P(edge<0) | P(−50% DD) | walk-forward |
|---|---|---|---|---|---|---|---|---|---|
| W=8 long-only, 1x | **1.02** | 20.5% | −23.3% | 3.07 | 0.91 | **0.60** | 0.04% | 0.1% | full-sample pick (biased) |
| W=8 long-only, 2x | 1.02 | 45.2% | −41.2% | 3.07 | 0.91 | 0.60 | 0.00% | **14.8%** | — |
| **ensemble W2–30, 1x** | 0.83 | 15.1% | −24.9% | 2.51 | 0.73 | 0.35 | 0.13% | 0.1% | **passes, eff. 0.94** |
| ensemble W2–30, 1.7x | 0.83 | 26.9% | −38.5% | 2.51 | 0.73 | 0.35 | 0.10% | 6.9% | — |
| buy & hold | 0.79 | 19.9% | −35.6% | 2.38 | 0.90 | 0.45 | 0.50% | 4.6% | — |

Leverage adds ruin probability (0.1% → 6.9–14.8%) without adding a basis
point of Sharpe — rejected at any multiple above 1x.

**3. Cluster placement.** Both finalists sit inside the robust family from
the cluster study (correlation with the family centre: ensemble 1.00, W=8
0.89) and are strongly anti-correlated (−0.65 to −0.77) with the toxic
momentum clusters.

**Verdict.** The ensemble is the only candidate whose out-of-sample evidence
is untainted by selection: nothing in it was fitted, and it walked forward at
efficiency 0.94 while single-W selection collapsed to 0.16. W=8's prettier
full-sample line is exactly what a full-sample pick looks like. The shipped
best version is therefore **long-only plateau ensemble W 2..30, 1x, never
...
python3 robustness_analysis.py # param stability, MC per set, clusters, walk-forward
