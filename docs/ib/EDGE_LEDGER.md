...
| 2026-08-30 | The pasted Turtle script with System 2 removed, the four requested filters added, and its own two gates swept in both directions (4,644,864 cells) | **nothing clears the selectivity control on all three blocks** | the script's own `ADX < 22` reads p 0.940 on held-back US30, where a random filter of the same selectivity earned MORE; both gates together p 0.983 | the ADX inversions clear US30 (p 0.014/0.011) and FAIL research (0.334/1.000) — the wrong shape; V51's MA200 floor goes p 0.001 on 60m Donchian to **p 0.994** on 240m Turtle, so a filter belongs to a GEOMETRY not a market; absorption's sign flips between timeframes on n=22 | why does the same gate invert at 15m and not at 240m — sweep the gate against BAR SPEED |
