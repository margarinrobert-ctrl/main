...
| --- | --- | --- | --- | --- | --- |
| E1 | **Short-horizon mean reversion at the EXECUTION layer**: a resting limit 0.75–1.0×ATR(5) against the move, instead of a market order, on a trade you were making anyway | `STUDY_LIMIT_ENTRY`, `STUDY_ATME`, `STUDY_V10_LIMIT`, `STUDY_V14_WINDOW_GRID` | +$4 to +$38/trade on EVERY bar with no rule, both sides, both blocks, four markets; +0.24 to +0.43 R over a market order on the same configuration | a signal (0.28 ticks vs a 6-tick round turn as a predictor); an add-on to a good signal (it destroys immediacy edges: $55k → $13k on the nine legs) | costs two-thirds of the fills; must be settled on the 1-minute path (`limit_entry.run_1m`), bar-level backtests overstate it 5× |
...
5. **It is small.** +0.05 to +0.30 R per trade. Anything larger on this branch has been a leak, a
