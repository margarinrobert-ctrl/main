...
def build(keys: list[str], path: str, title: str, subtitle: str) -> list:
    ...
    fp = (
        ...
        "//\n"
        "// AND: 87% of that holdout profit is MARKET BETA.  Regress the session P&L on the\n"
        "// index's own 07:00-11:00 move and the Sharpe falls 0.222 -> 0.032; $16,789 of profit is\n"
        "// $2,147 of alpha.  A second search over 901,120 more configurations, ranked directly on\n"
        "// market-neutralised Sharpe, found lower-beta versions that made money only on the block\n"
        "// they were selected on -- selection-block to validation-block correlation -0.057.\n"
        "//\n"
